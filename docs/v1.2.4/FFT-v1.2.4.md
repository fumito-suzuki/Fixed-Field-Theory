...
Modern physics consists of two extraordinarily successful yet conceptually
disconnected theories:
	•	Quantum mechanics, describing the small
	•	General relativity, describing the large

Between them lies a structural incompatibility:
	•	QM is fundamentally linear and operator-based
	•	GR is nonlinear and geometric

Furthermore, neither theory accounts for:
	•	The structure of perception
	•	The emergence of temporal order
	•	The stability of causal direction
	•	The relationship between information and geometry

Attempts at unification—string theory, loop quantum gravity, causal sets,
holography—each capture important aspects, yet rely on strong assumptions:
	•	Discrete lattices
	•	Higher-dimensional manifolds
	•	New particles or new symmetries
	•	Background dependence

FFT takes a different route:
postulating a single geometric substrate from which all known physics emerges.

### 1.2 Core Idea of Fixed-Field Theory

FFT posits that physical reality is supported by a continuous fractal causal field
called the T-fiber.

Key properties:
	•	It is not a lattice
	•	It is continuous, but fractal in the Z-direction (τ-time direction)
	•	It accumulates causal influence (“stacking”)
	•	Its local phase alignment produces what is perceived as time’s solidity
	•	Disturbances propagate as T-waves

The observable universe arises not from the T-fiber alone, but from its
interaction with a complementary mode:
	•	The C-wave, representing receptor-dependent, concept-forming interference
	•	All observation occurs via C-wave projection onto the XY-plane
	•	Two C-wave projections (XZ, YZ) fuse into the 2.5D perceptual world

The interaction between T and C is encoded by:

$$
\chi : \text{causal accumulation along Z},\quad
\delta : \text{interference density on XY},
$$

and the unified expression

$$$
\psi = \chi\delta
$$

generates the information-density gradient from which physical laws derive.

### 1.3 What FFT Is — and Is Not

FFT is:
	•	A geometric unification framework, not a particle theory
	•	A continuous (non-lattice) field theory
	•	A fractal time-direction geometry, not discrete time
	•	A deterministic field evolution with observer-dependent projections
	•	A theory in which QM emerges rather than being postulated

FFT is not:
	•	A metaphysical claim (“everything is vibration”)
	•	A simulation hypothesis
	•	A claim that the universe is literally 2D
	•	A replacement for empirical physics
	•	A discrete lattice model

These clarifications address common misunderstandings (AI models included)
and ensure FFT is interpreted within its proper mathematical scope.

### 1.4 Contributions of This Work

This paper provides:

#### (1) A minimal dynamical system

Three equations (FFT-1–3) forming the backbone of all FFT dynamics.

#### (2) A geometric derivation of physical constants

$$
Including \hbar, G, and \lambda.
$$

#### (3) A unified account of quantum phenomena
...
#### (4) A geometric perception model

Explaining depth, shadow, spherical appearance, and 2.5D observation.

#### (5) A cosmological model without dark-sector particles

Dark matter/energy arise from T-fiber tension variations.

#### (6) A mathematical model of consciousness
...
- Section 2 develops T-fiber geometry and χδ interaction
- Section 3 introduces FFT-1/2/3
- Section 4 derives perceptual geometry (2.5D)
- Section 5 derives quantum mechanics from χδ
- Section 6 derives cosmology
- Section 7 presents consciousness and free will
- Section 8 discusses interpretations
- Section 9 concludes and notes open problems

## 2. Foundations of Fixed-Field Geometry

The Fixed-Field Theory is built upon a continuous fractal causal substrate,
the T-fiber, and its interaction with a complementary interference field,
the C-wave.
This section formalizes their mathematical structure, clarifies their physical roles,
and derives the core composite quantity:

$$
\psi = \chi\delta .
$$

This ψ-field will later generate the information density ρ and govern all observable
dynamics through FFT-1〜3.

### 2.1 The T-fiber: A Continuous Fractal Causal Field

#### 2.1.1 Definition

The T-fiber is defined as a continuous field
T(x,y,z,t)
that encodes the accumulation of causal influence along a preferred direction
denoted by z or $\tau$ (phase-time).
...
There is no discretization of spacetime.
Instead it is modeled as a continuous differentiable field, except where
fractal scaling induces non-integer differentiation orders.

##### (B) Fractality along Z (τ-time)

Although continuous, T exhibits self-similar scaling in the Z-direction:

$$
T(x, y, \lambda z, t) \sim \lambda^{D} T(x, y, z, t),
$$

with fractal dimension D satisfying 1 < D < 3.

##### (C) Causal stacking

At each spatial point (x,y), T-fiber “accumulates” causal influence:

$$
\partial_t T = \chi(x,y,t) + \text{wave terms}.
$$

The quantity $\chi$ represents causal accumulation rate.

##### (D) Phase alignment produces emergent time

When local regions of T-fiber satisfy:

$$
\partial_z T \approx \text{const.},
$$

their evolution yields a stable temporal ordering, experienced phenomenologically
as “the flow of time”.

This motivates the definition:

Definition: τ-time (phase-time)

$$
\tau(x,y,z,t) = \text{local phase of } T(x,y,z,t).
$$

Time in FFT is not a fundamental dimension but an emergent ordering resulting from
local phase alignment in the T-fiber.

⸻

### 2.2 The C-wave: Receptor-Dependent Conceptual Interference

#### 2.2.1 Physical meaning

C-wave is the mathematical representation of:
- measurement
- perception
- conceptual structure
- information extraction

and corresponds to the observer-dependent component of reality.

Unlike T-wave (continuous), C-wave is receptor-based:
it exists only when there is a system capable of absorbing or transducing the wave
(eye, sensor, molecule, etc.).

Thus:
- Sound is T-wave → C-wave (pressure → cochlea)
- Light is T-wave → C-wave（electromagnetic → retina）
- Smell, taste, touch also arise through receptor-triggered C-wave fields
...
is defined on the observer’s XY-plane projection, obtained via:

$$
C = \Pi_{XY}[\text{TC-wave interference}].
$$

C is not defined along Z; instead, Z-depth enters via delayed phase
(important later in Section 4).

### 2.3 χ and δ: The Fundamental Operators

FFT introduces two primitive geometric quantities.

#### 2.3.1 χ — Causal Accumulation

$$
\chi(x,y,t) = \frac{\partial T}{\partial z}.
$$

This measures how much causal tension accumulates along the T-fiber at location (x,y).

Physical meaning:
- high χ → dense causal history
- low χ → “thin” causal structure

#### 2.3.2 δ — Interference Density

δ quantifies how C-wave interference modifies accessible information in the XY-plane:

$$
\delta = |\nabla C|^2 \quad \text{or generalized forms}.
$$

Physical meaning:
- high δ → strong perceptual influence
- δ = 0 → no observation / no measurement

δ captures everything from visual edges to conceptual gradients.

### 2.4 ψ = χδ: The Unified Information-Gradient Field

The central unifying quantity in FFT is:

$$
\boxed{\psi(x,y,t) = \chi(x,y,t)\, \delta(x,y,t)}.
$$

Why multiply?
- χ gives causal depth
- δ gives informational contrast

Their product yields how strongly information propagates into T-fiber structure.

This ψ-field appears in:
- FFT-1 (nonlinear term)
- FFT-2 (continuity equation source)
- FFT-3 (C-wave projection dynamics)
- entanglement geometry
- cosmological curvature
- free will / consciousness

### 2.5 Information Density ρ and Its Relation to ψ

FFT defines information density ρ via:

$$
\partial_t \rho = \beta_1 \|\nabla \psi\|^2 + \beta_2 \|\nabla C\|^2 - \gamma\rho.
$$

Interpretation:
- ψ-gradients increase ρ（self-organization）
- C-wave gradients reinforce ρ（perception → structure）
- $\gamma$ controls dissipation

ρ is not “mass density” but informational tension density,
yet mass emerges from ρ in specific limits (shown in Section 5 and 6).
...
| δ | C-wave interference density | XY-plane | gives contrast |
| ψ | χδ | XY-plane → T-fiber | unified info gradient |
| ρ | information density |	3D | mass/structure precursor |
| τ | emergent time | from phase alignment | not primary |
